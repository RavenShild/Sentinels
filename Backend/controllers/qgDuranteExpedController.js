...
router.get("/qg_durante_expediente", (req, res) => {
    const sql = "SELECT distinct pde.id, pde.pg, pde.nomeGuerra, pde.idtMil, pde.dataEntrada, pde.horaEntrada, pde.horaSaida, pde.origem FROM qg_durante_expediente pde INNER JOIN config_servico cs ON pde.config_servico_id = cs.id WHERE cs.configurado = 1 order by pde.dataEntrada, pde.horaEntrada";
...
router.get("/servico_anterior_qg_durante_expediente", (req, res) => {
    const sql = "SELECT * FROM bk_qg_durante_expediente order by dataEntrada, horaEntrada";
    db.query(sql, (err, data) => {
        if (err) return res.json(err);
        return res.json(data);
    });
...
router.post("/qg_durante_expediente", (req, res) => {
    ...
    const sql = "INSERT INTO qg_durante_expediente (pg, nomeGuerra, idtMil, dataEntrada, horaEntrada, horaSaida, origem, config_servico_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
...
router.get("/qg_durante_expediente/selectId/:id", (req, res) => {
    const id = req.params.id;
    const sql = "SELECT * FROM qg_durante_expediente WHERE id = ?"; // Consulta SQL para buscar o registro pelo ID
...
router.put("/qg_durante_expediente/:id", (req, res) => {
    const id = req.params.id;
    ...
    const sql = "UPDATE qg_durante_expediente SET pg=?, nomeGuerra=?, idtMil=?, dataEntrada=?, horaEntrada=?, horaSaida=?, origem=? WHERE id=?";
...
router.delete("/qg_durante_expediente/:id", (req, res) => {
    const civisId = req.params.id;
    const sql = `DELETE FROM qg_durante_expediente WHERE id = ?`;
